...
double beta_pow_factorial[qmax]; // contains the values beta^k / k!
...
double currWeight; int TstepsFinished = 0;
...
double meanq = 0;
double maxq = 0;
double r_saved, rdiff_saved, rdiff2_saved;
double start_time;
...
void save_QMC_data(int printout = 1){
	...
	fout(currD); fout(old_currD); fout(currD_partial); fout(currWeight); fout(step); fout(measurement_step); 
...
void load_QMC_data(){
	...
	if(finput.good()){
		...
		fin(currD); fin(old_currD); fin(currD_partial); fin(currWeight); fin(step); fin(measurement_step); 
...
void GetEnergies(){
	currD = currD_partial[0] = 1; meanE = 0.0;
	for(int i=0;i<q;i++){
	...
	meanE = q>0 ? meanE/q : currEnergy;
}

int EvaluatesTo1(int len){
	currz.reset();
	for(int i=0;i<len;i++) currz = P_matrix[Sq[i]]^currz;
	return currz.none();
}

int RepetitionCheck(){
	int i,j,rep=0;
	for(i=1;i<q;i++) if(q%i==0){
		j=i; while(j<q && Sq[j]==Sq[j-i]) j++;
		if(j==q) if(EvaluatesTo1(i)){ rep = 1; break;}
	}
	return rep;
}

void w(double D, double &r){
	double p_coeffs1[q+1], p_coeffs2[q+1]; double a1, a2, multiplier = exp(-beta*meanE); int i;
	r = 0.0; p_coeffs1[0] = 1.0;
	double divided1[q], divided2[q], der_array1[q+1], der_array2[q+1], p_der_coeffs[q];
	for(i = 0; i < q; i++){
		for(int j = 0; j < i + 1 ; j++) p_coeffs1[i+1 - j] = p_coeffs1[i-j]; p_coeffs1[0] = 0.0;
		for(int j = 0; j < i + 1; j++){ p_coeffs1[j] += (Energies[i]-meanE)*p_coeffs1[j+1]; p_coeffs2[j] = p_coeffs1[j];}
	}
	p_coeffs2[q] = p_coeffs1[q]; p_coeffs1[0] += (q%2*2-1)*D;
	for(i = 0; i < q; i++) p_der_coeffs[i] = (i+1)*p_coeffs1[i+1];
	for(i = 0; i < q; i++) divided1[i] = divided2[i] = p_der_coeffs[i];
	for(i = 0; i < kmax; i++){
		a1 = divided1[q-1]/p_coeffs1[q]; a2 = divided2[q-1]/p_coeffs2[q];
		for(int j=0; j < q+1; j++)
			if(j == 0){
				der_array1[j] = -1.0*a1*p_coeffs1[j];
				der_array2[j] = -1.0*a2*p_coeffs2[j];
			}
			else{
				der_array1[j] = -1.0*a1*p_coeffs1[j] + divided1[j-1];
				der_array2[j] = -1.0*a2*p_coeffs2[j] + divided2[j-1];
			}
		a1 -= a2;
		r += beta_pow_factorial[i]*a1;
		for(int j = 0; j < q; j++){
			divided1[j] = der_array1[j];
			divided2[j] = der_array2[j];
		}
	}
	r *= multiplier/q;
}

void w(double D, double &r, double &rdiff, double &rdiff2){
	double p_coeffs1[q+1], p_coeffs2[q+1]; double a1, a2, multiplier = exp(-beta*meanE); int i;
	r = rdiff = rdiff2 = 0.0; p_coeffs1[0] = 1.0;
	double divided1[q], divided2[q], der_array1[q+1], der_array2[q+1], p_der_coeffs[q];	
	for(i = 0; i < q; i++){
		for(int j = 0; j < i + 1 ; j++) p_coeffs1[i+1 - j] = p_coeffs1[i-j]; p_coeffs1[0] = 0.0;
		for(int j = 0; j < i + 1; j++){ p_coeffs1[j] += (Energies[i]-meanE)*p_coeffs1[j+1]; p_coeffs2[j] = p_coeffs1[j];}
	...
	for(i = 0; i < kmax; i++){
		...
		a1 -= a2; r += beta_pow_factorial[i]*a1;
		if(i>0) rdiff  -= beta_pow_factorial[i-1]*a1;
		if(i>1) rdiff2 += beta_pow_factorial[i-2]*a1;
		for(int j = 0; j < q; j++){
			divided1[j] = der_array1[j];
			divided2[j] = der_array2[j];
		}
	}
	r/=q; rdiff/=q; rdiff2/=q;
	rdiff2 = multiplier*(rdiff2 + 2*meanE*rdiff + meanE*meanE*r);
	rdiff = multiplier*(rdiff + meanE*r);
	r *= multiplier;
}

double GetWeight(){
	double r;
	if(RepetitionCheck()) return 0; else{
		GetEnergies();
		if(q==0) r = exp(-beta*currEnergy); else w(currD.real(),r); // this should be corrected to account for complex D
		return r;
	}
}

inline double UpdateWeight() { return GetWeight(); }
inline double UpdateWeightReplace(double removeEnergy, double addEnergy) { return GetWeight(); }
inline double UpdateWeightDel(double removeEnergy1, double removeEnergy2) { return GetWeight(); }
inline double UpdateWeightIns(double addEnergy1, double addEnergy2) { return GetWeight(); }
...
void init_basic(){
	double curr1=1, curr2=1; beta_pow_factorial[0] = curr1; factorial[0] = curr2;
	for(int k=1;k<qmax;k++){ curr1*=beta/k; curr2*=k; beta_pow_factorial[k] = curr1; factorial[k] = curr2;}
	currWeight = GetWeight();
}

void init(){
	int i,j; double curr1=1, curr2=1; beta_pow_factorial[0] = curr1; factorial[0] = curr2;
...
double Metropolis(double newWeight){
	return min(1.0,fabs(newWeight/currWeight));
}

void update(){
	if(save_data_flag){ save_QMC_data(); exit(0); };
	int i,m,p,r,u,oldq,cont; double oldE, oldE2, v = Nop>0 ? val(rng) : 1; double newWeight; double Rfactor;
...
double measure_H(){
	return rdiff_saved/r_saved;
}

double measure_H2(){
	return rdiff2_saved/r_saved;
}

double measure_Hdiag(){ return 0; }

double measure_Hdiag2(){ return 0; }

double measure_Hoffdiag(){ return 0; }

double measure_Hoffdiag2(){ return 0; }
...
void measure(){
	double sgn, R; int i; GetEnergies();
	if(q==0){
		r_saved = exp(-beta*currEnergy);
		rdiff_saved = currEnergy*r_saved; 
		rdiff2_saved = currEnergy*rdiff_saved;
	} else w(currD.real(),r_saved,rdiff_saved,rdiff2_saved);
	sgn = (r_saved > 0.0) - (r_saved < 0.0);          // correct the above line for the case of complex D
